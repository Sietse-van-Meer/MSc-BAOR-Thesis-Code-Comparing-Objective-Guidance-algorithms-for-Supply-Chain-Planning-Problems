...
@njit(cache=True)
def simplified_random_local_search(jobs, c_t, max_time, seed_value, max_plateau_moves, phase, start_iteration, nu, max_iterations, best_original_objective_value=np.inf):
    ...
    best_original_objective_value = min(best_original_objective_value, current_original_objective)  # Keep best found original objective
...
def local_search_phase_sequence(jobs, c_t, max_time, sequence, max_plateau_moves, nu, seed_value):
    ...
    all_iterations_trace = []
    best_objective_overall = np.inf
